...
<#
    .SYNOPSIS
        Packages the latest VMware Horizon for Microsoft Endpoin Manager (Intune) deployment.
...
Param (
    ...
    [System.String] $Path = "D:\MemApp\VmwareHorizon",
    ...
    [Parameter(Mandatory = $False)]
    [System.String] $TenantName = "geckogreen.onmicrosoft.com",
    ...
    [Parameter]$PackageName="VMwareHorizonClient"
)
...
ForEach ($Arch in $Architecture) {
    ...
    #region Variables
    Write-Host -ForegroundColor "Cyan" "Getting $PackageName $Arch version via Evergreen."
    $ProgressPreference = "SilentlyContinue"
    $InformationPreference = "Continue"
    
    if ($Language -eq "")
    {
        $Package = Get-EvergreenApp -Name $PackageName | Select-Object -First 1    
    }
    else{
        $Package = Get-EvergreenApp -Name $PackageName | Where-Object { $_.Language -eq "English" } | Select-Object -First 1
    }

    $res = Export-EvergreenManifest -Name $PackageName
