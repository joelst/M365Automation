...
<#
.SYNOPSIS
  Ultimate Windows IR Initial Collection — Hybrid v3.0
.DESCRIPTION
  Standard elevated variant. Forensic-first, read-only, SYSTEM-ready (32/64-bit safe). Full volatile (network, accounts, drivers, ETW, BITS, named pipes),
...
.NOTES
  ...
  Standard variant: includes #Requires -RunAsAdministrator and strict mode.
  Evidence output path: C:\ProgramData\IR
...
<#
.SYNOPSIS
    Adds a SHA256 hash record to the manifest for a file.
.PARAMETER filePath
...
<#
.SYNOPSIS
    Records a posture finding and optional warning summary.
.PARAMETER category
    Finding category.
.PARAMETER item
    Finding item label.
...
.PARAMETER status
    Status label such as OK, INFO, WARNING, CRITICAL.
#>
...
<#
.SYNOPSIS
    Returns analyst next-step guidance for an offline hint.
.PARAMETER category
    Finding category.
.PARAMETER item
    Finding item label.
...
<#
.SYNOPSIS
    Adds an analyst-facing offline hint entry.
.PARAMETER priority
    Hint priority (HIGH/MEDIUM/LOW).
.PARAMETER category
    Finding category.
.PARAMETER item
    Finding item.
...
<#
.SYNOPSIS
    Reads a registry value from both 64-bit and 32-bit views.
.PARAMETER regPath
    Registry path under HKLM.
.PARAMETER valueName
...
<#
.SYNOPSIS
    Executes a script block safely and captures output if needed.
.PARAMETER description
    Description used for logging and findings.
.PARAMETER outputPath
    Optional file path to write command output when the block does not already create a file.
...
function Invoke-SafeCommand {
  ...
  try {
    ...
    $msg = "Unavailable: $_"
    Write-Warning "ERROR $description`: $msg"
    if ($outputPath) {
...
<#
.SYNOPSIS
    Executes a section and continues on terminating errors.
.PARAMETER description
    Description used for logging and findings.
.PARAMETER scriptBlock
    Script block to invoke.
...
<#
.SYNOPSIS
    Runs a script block with PSDefaultParameterValues cleared.
.PARAMETER scriptBlock
    Script block to invoke.
#>
...
<#
.SYNOPSIS
    Copies a file that may be locked by another process.
.PARAMETER sourcePath
    Full path to the source file.
.PARAMETER destinationPath
    Full path to the destination file.
.PARAMETER description
    Description used for findings/logging.
#>
...
Invoke-SafeSection -description 'Defender Posture' -scriptBlock {
  ...
  # Expanded Defender posture (matches hardening script)
  try {
...
Invoke-SafeSection -description 'Artifacts' -scriptBlock {
  ...
  # WMI Repository (persistence detection)
  ...
  # PowerShell Console History (all user profiles accessible from SYSTEM)
  ...
  # Browser History (SQLite DB copies)
  ...
  # Certificates (suspicious certs in root/trusted stores)
  ...
  New-Folder 'Artifacts\RemoteAccess' | Out-Null
  $remoteSearchRoots = @(
...
Invoke-SafeSection -description 'Finalize' -scriptBlock {
  ...
  # Standalone sidecar for ultra-pedantry
