...
<#
    .SYNOPSIS
        Packages the latest Microsoft WinDbg (Preview) from the Microsoft Store for Microsoft Endpoint Manager (Intune) deployment.
...
Param (
    ...
    [Parameter(Mandatory = $False)]
    $AppPath = "$env:ProgramFiles\WindowsApps\Microsoft.WinDbg_8wekyb3d8bbwe",

    [Parameter(Mandatory = $False)]
    $AppExecutable = "WinDbg.exe",
