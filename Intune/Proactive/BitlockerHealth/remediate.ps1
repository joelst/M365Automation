...
Start-Transcript -Path "$env:TEMP\BitLocker-$(Get-Date -f MMddHHmm).txt"

# Check Bitlocker prerequisites
$TPMNotEnabled = Get-CimInstance win32_tpm -Namespace root\cimv2\security\microsofttpm | Where-Object { $_.IsEnabled_InitialValue -eq $false } -ErrorAction SilentlyContinue
$TPMEnabled = Get-CimInstance win32_tpm -Namespace root\cimv2\security\microsofttpm | Where-Object { $_.IsEnabled_InitialValue -eq $true } -ErrorAction SilentlyContinue
$WindowsVer = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue | Where-Object {
...
# Step 1 - If TPM is present but not enabled, initialize it.
...
if (Test-Path "$BitLockerRegLoc\FVE" -and $TPMNotEnabled) {
    ...
    New-Item -Path "$BitLockerRegLoc" -Name 'FVE' -Force | Out-Null
