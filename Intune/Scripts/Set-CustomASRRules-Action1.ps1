...
# Action1 RMM version - no param() block (Action1 doesn't support CmdletBinding).
# Hardening logic below the SHARED CORE marker is identical to Set-CustomASRRules.ps1.
# Author: Joel Stidley | CIS Benchmark: Microsoft Windows 11 Enterprise v3.0.0
...
# Set-CustomASRRules.ps1. Keep both files in sync when editing.
# ========================================================================
$SetReg = {
    param(
        [string]$Path,
...
& $SetReg -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'ConsentPromptBehaviorUser' -Value 1 -Type 'DWORD'
...
& $SetReg -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient' -Name 'EnableMulticast' -Value 1 -Type 'DWORD'
...
# Harmless add if doesn't exist, ensures path is present for future changes
...
& $SetReg -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop' -Name 'ScreenSaveTimeOut' -Value '400' -Type 'String'
