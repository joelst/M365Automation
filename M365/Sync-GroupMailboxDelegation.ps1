...
#>
    ...
    param (
        ...
        [Parameter()]
        [string]
        $AADCertificateThumbprint = $env:EXO_CERT_THUMBPRINT,
        [parameter()]
        [string]
        $ExchangeClientAppId = $env:EXO_APP_ID,
        [Parameter()]
        [string]
    ...
    # Resolve configuration for authentication/tenant context without hardcoded defaults
    if (-not $AADCertificateThumbprint) {
        $AADCertificateThumbprint = $env:AAD_CERTIFICATE_THUMBPRINT
    }

    if (-not $ExchangeClientAppId) {
        $ExchangeClientAppId = $env:EXCHANGE_CLIENT_APP_ID
    }
    ...
    if (-not $AADCertificateThumbprint -or -not $ExchangeClientAppId -or -not $TenantId) {
        throw "Sync-GroupMailboxDelegation: AADCertificateThumbprint, ExchangeClientAppId, and TenantId must be provided either as parameters or via environment variables (AAD_CERTIFICATE_THUMBPRINT, EXCHANGE_CLIENT_APP_ID, TENANT_ID)."
    }
...
#>
        ...
        begin {
            try {
                if ($null -eq (Get-AzAccessToken)) {
                    Write-Host "Log in with Connect-AzAccount first"
                    Connect-AzAccount -CertificateThumbprint $AADCertificateThumbprint -ApplicationId $ExchangeClientAppId -Tenant $TenantId
                }
            }
            catch {
